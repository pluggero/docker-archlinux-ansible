...
### Step 3: Verify the image checksum

```bash
# Pull and save the image
docker pull pluggero/docker-archlinux-ansible:$VERSION
docker save pluggero/docker-archlinux-ansible:$VERSION -o archlinux-ansible.tar

# Calculate the SHA256 of your saved image
sha256sum archlinux-ansible.tar

# Compare with the checksum in checksums.txt
cat checksums.txt
```

The checksums must match. If they don't, the image was modified after the build.

### What This Proves

- The checksums file was generated by our GitHub Actions pipeline (attestation)
- The image matches the checksum from the pipeline (manual verification)
